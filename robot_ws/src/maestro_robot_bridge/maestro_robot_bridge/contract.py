...
def parse_command(raw: str | bytes, now: datetime | None = None) -> Command:
    ...
    expires = payload["expires_in_ms"]

    if type(expires) is not int or not 1 <= expires <= 30000:
        raise ContractError(
            "expires_in_ms must be between 1 and 30000",
            command_id,
        )

    intent = payload["intent"]

    if intent not in {"SPRAY", "DOCK", "UNDOCK"}:
        raise ContractError(
    ...
    if intent == "SPRAY":
        if not isinstance(target_payload, dict):
            raise ContractError(
                "SPRAY requires mapped target",
                command_id,
            )

        if set(target_payload) != {"type", "id"}:
            raise ContractError(
                "target must contain only type and id",
                command_id,
            )
        ...
        raise ContractError(
            "DOCK and UNDOCK cannot have target",
            command_id,
        )
    ...
    if created_at > current + timedelta(seconds=5):
        raise ContractError(
            "command timestamp is in the future",
            command_id,
        )

    expires_at = created_at + timedelta(milliseconds=expires)
