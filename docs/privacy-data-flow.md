...
| Maestro — consultas de histórico e estado | `plot_id` quando aplicável, UUID técnico, tipo de query e estado/resultado da operação simulada | Regras locais `PLOT_STATUS_QUERY` e `STATUS_QUERY`, rota `/read-only` | Consulta em memória ao bridge; TTS/UI recebem resposta tipada | Nenhuma persistência no app; bridge mantém no máximo 100 registros efêmeros sem fala, mídia, produto, dose ou coordenada bruta | Implementado no `mockDebug`/Gazebo; não é histórico de aplicação física |
