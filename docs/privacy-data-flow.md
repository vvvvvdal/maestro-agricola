...
| Maestro — consulta de histórico | `plot_id`, UUID, horário, tipo de query e resposta de operação simulada | Regra local `PLOT_STATUS_QUERY` e rota `/read-only` | Consulta em memória ao bridge; TTS/UI recebem resposta tipada | Nenhuma persistência no app; bridge mantém no máximo 100 registros efêmeros sem fala, mídia, produto ou dose | Implementado no `mockDebug`/Gazebo; não é histórico de aplicação física |
