...
Para testar o lifecycle atual no Gazebo, use o app: se o robô estiver dockado, envie `UNDOCK` explicitamente; aguarde a tela mostrar a conclusão ou falha da saída; então envie e confirme `SPRAY`. A tela acompanha a operação aceita pela rota somente leitura e só mostra conclusão após a resposta terminal do bridge. Se não houver resposta após 30 consultas, mostra falha fechada e bloqueia novas ações até `Reiniciar`; verifique o Gazebo antes desse reinício. A JEV-73 validou esse caminho de falha fechada no tablet; repita a conclusão normal numa sessão dedicada do Gazebo antes de usá-la como evidência de navegação. Verifique que a navegação termina no plot sem retorno automático; envie `DOCK` explicitamente somente quando quiser retornar. Não use o HMI manual como parte do E2E Maestro.
