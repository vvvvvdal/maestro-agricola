...
| --- | --- | --- | --- |
| JEV-69 | DONE | JEV-38 | Fronteira de minimizacao especificada em [`../../tasks/jev-remote-privacy-gate.md`](../../tasks/jev-remote-privacy-gate.md): mapa de dados, ameacas, UX de autorizacao de sessao, retorno ao modo Local e criterios de bloqueio antes da rede. O caminho padrao continua local e `dat` nunca usa Jev remoto. |
| JEV-69A | DONE | JEV-69 | `RemoteTranscriptGate` no `mockDebug` e defesa equivalente no proxy bloqueiam dados pessoais evidentes, URL, comprimento ou fala fora do escopo antes do HTTP. O aviso de ativacao explicita o que nao compartilhar e a permissao vale somente na sessao, podendo ser revogada. Uma fala bloqueada nao reutiliza Qwen, WebSocket ou `Command`. Testes Kotlin/Python focados e `assembleMockDebug` passaram. |
| JEV-69B | DONE | JEV-69A | Validada no SM-X510 em 25/09/2026 com frases sinteticas: aceite de sessao, fala operacional aceita pelo caminho remoto sem novo aviso, CPF sintetico bloqueado localmente como `Fala nao enviada ao Jev` e revogacao de volta para `Local`. A evidencia limita a afirmacao a minimizacao preventiva; nao e anonimização nem conformidade integral. |
...
| ID | Status | Dependencia | Entrega e criterio de aceite |
| --- | --- | --- | --- |
| JEV-70 | DONE | JEV-69B | Contrato de leitura em [`../../tasks/read-only-query-contract.md`](../../tasks/read-only-query-contract.md): `ReadOnlyQuery`, `OperationRecord`, resposta narravel, retencao somente em memoria e fronteira sem `Command`. No Gazebo, a evidencia e chegada Nav2 de uma missao simulada de pulverizacao; nao e aplicacao fisica comprovada. |
| JEV-71 | DONE | JEV-70 | `OperationHistory` no bridge mantem ate 100 `OperationRecord` somente em memoria. So `_complete_navigation(succeeded=True)` de `SPRAY` registra chegada Nav2 simulada; falha nao registra, callbacks repetidos deduplicam e nova sessao inicia vazia. Testes focados passaram. |
| JEV-72 | DONE | JEV-70, JEV-71 | `PLOT_STATUS_QUERY` local reconhece pergunta com talhao explicito, consulta `/read-only` e narra `FOUND`, `NOT_FOUND`, `INVALID_QUERY` ou `UNAVAILABLE`. Usa trilha propria no app e nunca chama Jev, Qwen, `CommandTransport` ou ROS de movimento. Testes Python/Kotlin focados e `assembleMockDebug` passaram. |
| JEV-73 | DONE | JEV-70 | `STATUS_QUERY` e acompanhamento somente leitura implementados: o app distingue enfileirada, em execucao, concluida, falha e indisponivel pelo mesmo `command_id`, sem emissao adicional de `Command`. No tablet, a consulta leu `EXECUTING` e o timeout de 30 consultas chegou a falha fechada, bloqueando novos comandos ate `Reiniciar`. Testes unitarios cobrem `COMPLETED`; a validacao E2E de conclusao normal fica para sessao dedicada com Gazebo saudavel. |
| JEV-74 | DONE | JEV-70 | `INSPECT_TARGET` e uma acao local explicita de captura sob demanda: QR/marcador em memoria, resultado tipado e descarte da imagem. Nao e uma setima classe Jev e nao cria `Command`. Os testes Kotlin cobrem a transicao sem comando e a politica de QR; no SM-X510, `mockDebug` mostrou `Marcador plot-03 identificado`. Permissao negada, QR invalido e timeout permanecem falhas fechadas na fronteira DAT/decoder. |
| JEV-75 | DONE | JEV-70, JEV-72, JEV-73, JEV-74 | `ReadOnlyLanguageRouter` local roteia `PLOT_STATUS_QUERY`, `STATUS_QUERY`, `INSPECT_TARGET` e recusa `NONE`. O corpus sintetico de desenvolvimento, balanceado em 32 casos, passou 32/32 no teste Kotlin; corrigiu a fronteira `situacao da operacao -> STATUS_QUERY`. No SM-X510, `leia o qr code` abriu a captura mock e identificou `plot-03`, sem Jev, consulta ou `Command`. Nao usa API ou chave, nao e holdout nem evidencia de campo. Documento: [`../../tasks/read-only-language-router-benchmark.md`](../../tasks/read-only-language-router-benchmark.md). |
| JEV-76 | DONE | JEV-70, JEV-72 | Contrato em [`../../tasks/mission-preview-contract.md`](../../tasks/mission-preview-contract.md) e schema `contracts/mission-plan.schema.json`: `MISSION_PREVIEW` reconhece pedido composto, mas parser/validador deterministico produzem no maximo quatro passos tipados. O preview nao cria `Command`; cada acao fisica exigira confirmacao individual em JEV-77/JEV-78. Fixtures cobrem plano valido e alvo implicito invalido. |
| JEV-77 | DONE | JEV-76 | Parser/validador local e preview no `mockDebug` entregues: aceita somente a gramatica fechada do contrato, mostra duas a quatro etapas, distingue consulta de acao fisica futura, permite cancelar e expira em 60 s. Alvo ausente/desconhecido, ordem invalida e passo desconhecido sao recusados; nenhum ramo cria `Command`, chama Jev, Qwen, WebSocket, ROS ou `/read-only`. Testes Kotlin, `assembleMockDebug` e validacao no SM-X510 passaram. |
| JEV-78 | IN PROGRESS | JEV-77 | Executor deterministico local implementado: cada acao fisica gera `Command` com `command_id` proprio apenas apos confirmacao; consulta e falha pausam o plano sem proxima emissao. No SM-X510/Gazebo, o E2E completo executou `UNDOCK -> SPRAY plot-02 -> consulta plot-03 -> DOCK`, com confirmacoes individuais e narracao da consulta antes do retorno. O robo voltou para a doca em 79 s, mas a janela uniforme de 60 consultas gerou falso timeout; `DOCK` agora admite 120 consultas. Falta repetir o retorno e comprovar `COMPLETED` no app sem aviso. Nao usar Jev como planejador. |
